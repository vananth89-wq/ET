-- =============================================================================
-- Mig 644: fix upsert_personal_info — treat auth.uid() IS NULL as system path
--
-- When called from the apply_profile_pending_change trigger (SECURITY DEFINER,
-- no user session), auth.uid() returns NULL. The access guard then falls through
-- to user_can() / has_permission() which also return false with no session,
-- causing "Access denied".
--
-- Fix: if auth.uid() IS NULL, set v_is_system_path = true immediately.
-- Only SECURITY DEFINER functions/triggers can call a SECURITY DEFINER function
-- without a session, so this is safe.
-- =============================================================================
...
BEGIN
  ...
  -- No session = called from a SECURITY DEFINER trigger (e.g. apply_profile_pending_change)
  ...
  -- ── 1b. Layer-B fine-grained access guard (skipped for system paths) ─────────
  IF NOT v_is_system_path THEN
...
COMMENT ON FUNCTION upsert_personal_info(uuid, jsonb, date, boolean) IS
  'Mig 644: auth.uid() IS NULL → v_is_system_path = true. '
  'Allows SECURITY DEFINER trigger (apply_profile_pending_change) to call this '
  'function without a user session during workflow approval.';
