-- =============================================================================
-- Migration 240: Add `required` boolean to get_employee_hire_review fields
--
-- Mirrors the validation rules in AddEmployee.tsx so the hire review inline
-- editor can display a mandatory-field asterisk (*) next to required labels.
--
-- Required fields per section:
--   Personal:          Full Name, Nationality, Marital Status, Gender, Date of Birth
--   Contact:           Business Email, Personal Email, Mobile
--   Employment:        Designation, Department, Hire Date, Probation End, Work Country, Work Location
--   Address:           Line 1, Line 2, City, PIN, Country
--   Passport:          Passport Number, Issue Date, Expiry Date (conditional — marked required
--                      only when a passport country is already present)
--   Emergency Contact: Name, Relationship, Phone
--   Identity Docs:     not required by themselves (optional section)
-- =============================================================================
...
BEGIN
  ...
  v_result := v_result || jsonb_build_object(
    ...
    'fields', jsonb_build_array(
      ...
      ), 'raw_value',v_contact.mobile, 'key','contact.mobile', 'editable',true,'input_type','text','required',true)
    )
  );
