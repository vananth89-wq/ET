-- =============================================================================
-- Migration 182: Distinguish update vs. respond in the workflow action log
--
-- Previously wf_resubmit always logged action = 'resubmitted' regardless of
-- whether the employee actually changed data or just added a comment.
--
-- This patch logs:
--   'updated_and_resubmitted' — when p_proposed_data is provided (employee
--                               edited fields before resubmitting)
--   'resubmitted'             — when only a response comment was added
--
-- This lets the approver see in the timeline exactly what happened:
--   • "Employee updated and resubmitted" → they changed something
--   • "Employee resubmitted"             → they only added a clarification comment
-- =============================================================================
...
BEGIN
  ...
  -- 'updated_and_resubmitted' when the employee changed data (p_proposed_data
  -- was provided); 'resubmitted' when only a comment was added.
  v_action := CASE
...
-- Confirm the function body references both action values
SELECT proname,
