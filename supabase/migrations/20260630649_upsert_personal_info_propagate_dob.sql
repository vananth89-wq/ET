-- =============================================================================
-- Mig 649: upsert_personal_info — include dob in propagation
--
-- The propagation UPDATE (p_propagate=true) excluded dob and photo_url by
-- design. But when correcting a DOB mid-history and choosing "propagate to
-- future records", the user expects DOB to be fixed in all later slices too.
-- Add dob to the propagation SET clause.
-- photo_url is still excluded (per-record attachments should not auto-propagate).
-- =============================================================================
...
BEGIN
  ...
  IF v_case IS NULL THEN
    DECLARE v_first employee_personal%ROWTYPE; BEGIN
      ...
      IF FOUND AND p_effective_from < v_first.effective_from THEN
        v_case := 'prepend'; v_current_row := v_first;
      END IF;
    END;
  END IF;
  ...
  -- ── 7. Propagation — now includes dob ───────────────────────────────────────
  IF p_propagate THEN
    UPDATE employee_personal SET
      first_name = CASE
...
COMMENT ON FUNCTION upsert_personal_info(uuid, jsonb, date, boolean) IS
  'Mig 649: propagation now includes dob (previously excluded). '
  'photo_url still excluded — per-record attachments should not auto-propagate.';
