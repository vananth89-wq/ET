-- =============================================================================
-- Mig 632: Fix upsert_employment_info — add allow_employment_sync bypass
--          before syncing the employees mirror table.
--
-- Root cause: the employees sync UPDATE (step 11 in mig 616) runs without
-- setting prowess.allow_employment_sync = true. The guard trigger fires,
-- builds v_changed_mirror_cols || 'work_location' where v_changed_mirror_cols
-- is text[], PostgreSQL resolves || as text[]||text[] and fails to parse
-- 'work_location' as an array literal → "malformed array literal: work_location".
--
-- Fix: identical to mig 616's function body with one line added before the
-- UPDATE employees: PERFORM set_config('prowess.allow_employment_sync','true',true).
-- =============================================================================
