-- =============================================================================
-- Migration 299: bank effective-date rule hardening
--
-- Three changes:
--
-- 1. upsert_bank_account — effective_from must be EXACTLY the 1st of the
--    current month (not just "not in the past"). Standard ESS employees
--    submitting on/before the 15th may only request the current month's 1st.
--
-- 2. apply_profile_pending_change — approvers cannot approve a profile_bank
--    change after the 20th of the month (payroll cut-off). Exempt roles:
--    bank_exceptions, admin, hr.
--
-- 3. get_profile_workflow_gates — adds is_bank_exception: boolean to the
--    returned JSONB so the frontend can hide the "submit window closed"
--    banner for exempt users and suppress the 20th-day warning.
-- =============================================================================


-- =============================================================================
-- 1. upsert_bank_account — enforce current-month-1st only
-- =============================================================================
-- Only the date-validation block changes (lines formerly reading
-- "effective_from must not be in the past").  All other logic is unchanged.
-- The full function is re-created (CREATE OR REPLACE) to ensure the change
...
BEGIN
  ...
  IF NOT p_is_new_hire AND NOT v_is_bank_exception THEN
    ...
    -- ── CHANGED (mig 299): effective_from must be exactly the 1st of the
    -- current month — not a future month or any past date.
  ...
  ELSE
    ...
    -- Build proposed_data — used by apply_profile_pending_change when approved
    DECLARE
    ...
    BEGIN
      ...
      -- Create first-step task(s) via the standard task-seeding function
...
-- =============================================================================
-- 2. apply_profile_pending_change — add 20th-day approver block for profile_bank
-- =============================================================================
-- The full function is re-created. The profile_bank ELSIF branch gains a guard
-- at the top that raises an exception when day > 20 AND the approver is not
-- in an exempt role.
-- All other branches are unchanged from mig 275.
-- =============================================================================
...
BEGIN
  ...
  ELSIF v_module = 'profile_bank' THEN
    ...
    -- ── ADDED (mig 299): block approval after 20th unless exempt ────────────
...
COMMENT ON FUNCTION apply_profile_pending_change() IS
  ...
  'profile_emergency_contact → emergency_contacts, '
  'profile_bank → employee_bank_accounts (mig 275). '
  'profile_bank: blocks approval after day 20 unless caller has bank_exceptions/admin/hr (mig 299). '
  'profile_dependents → employee_dependents (mig 289). '
  'SECURITY DEFINER — runs as function owner to bypass RLS on satellite tables.';


-- =============================================================================
-- 3. get_profile_workflow_gates — add is_bank_exception to response
-- =============================================================================
...
BEGIN
  ...
  -- ── 4. ADDED (mig 299): bank exception flag ─────────────────────────────────
  -- True when the caller holds bank_exceptions, admin, or hr role.
  -- Frontend uses this to hide the "submission window closed" banner and skip
  -- the 20th-day approval warning for exempt users.
  v_is_bank_exception := has_role('bank_exceptions') OR has_role('admin') OR has_role('hr');
...
COMMENT ON FUNCTION get_profile_workflow_gates() IS
  ...
  'instance_ids for active workflow instances per module, and is_bank_exception flag. '
  ...
  'is_bank_exception: true when caller has bank_exceptions, admin, or hr role (mig 299). '
  ...
  'is_bank_exception added in mig 299.';
...
DO $$
BEGIN
  -- upsert_bank_account exists
  ASSERT (SELECT COUNT(*) FROM pg_proc WHERE proname = 'upsert_bank_account') >= 1,
    'upsert_bank_account not found after migration 299';

  -- apply_profile_pending_change exists
  ASSERT (SELECT COUNT(*) FROM pg_proc WHERE proname = 'apply_profile_pending_change') >= 1,
    'apply_profile_pending_change not found after migration 299';

  -- get_profile_workflow_gates exists
  ASSERT (SELECT COUNT(*) FROM pg_proc WHERE proname = 'get_profile_workflow_gates') >= 1,
    'get_profile_workflow_gates not found after migration 299';

  RAISE NOTICE 'Migration 299 verified: upsert_bank_account, apply_profile_pending_change, get_profile_workflow_gates all present.';
END;
$$;

-- =============================================================================
-- END OF MIGRATION 299
--
-- Frontend changes required (see summary in PR description):
--   • useProfileWorkflowGates.ts   — parse is_bank_exception from RPC response
--   • BankAccountsPortlet.tsx      — isBankException prop + pre-emptive cutoff banner
--   • MyProfile/index.tsx          — pass isBankException from hook to portlet
--   • EmployeeEditPanel.tsx        — pass isBankException={true} (HR always exempt)
--   • WorkflowReview.tsx           — 20th-day guard before calling approve() for profile_bank
-- =============================================================================
