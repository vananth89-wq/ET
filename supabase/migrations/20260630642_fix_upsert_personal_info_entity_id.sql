-- =============================================================================
-- Mig 642: fix upsert_personal_info — wi.entity_id → wi.record_id
--
-- Bug introduced in mig 623: the access guard section referenced wi.entity_id
-- which does not exist on workflow_instances. The correct column is wi.record_id.
-- This caused every call to upsert_personal_info (and its workflow wrapper)
-- to return {"ok":false,"error":"column wi.entity_id does not exist"}, silently
-- blocking all personal info workflow approvals from applying.
-- =============================================================================
...
BEGIN
  ...
  IF NOT v_is_system_path THEN
    IF EXISTS (
      ...
      WHERE wi.record_id   = p_employee_id          -- fix: was wi.entity_id
...
COMMENT ON FUNCTION upsert_personal_info(uuid, jsonb, date, boolean) IS
  'Mig 642: fixed wi.entity_id → wi.record_id in access guard (3 occurrences). '
  'This bug caused every workflow approval to silently fail with "column wi.entity_id does not exist".';
