-- =============================================================================
-- Mig 646: fix get_workflow_participants — SELF must resolve to submitter
--
-- Mig 641 introduced a self_emp_subject lateral that made SELF resolve to the
-- subject employee when p_subject_employee_id is supplied. This caused the CC
-- to show "Mohan Rajiv" twice (once for SELF, once for SUBJECT_EMPLOYEE).
--
-- Fix: SELF always resolves from p_profile_id (the submitter).
--      Only SUBJECT_EMPLOYEE resolves from p_subject_employee_id.
-- =============================================================================
...
BEGIN
    ...
    jsonb_build_object(
      ...
      'hasResolvedPerson',
        CASE ws.approver_type
          ...
          WHEN 'SUBJECT_EMPLOYEE' THEN (subject_emp.name IS NOT NULL)
          ELSE                         false
        END,
  ...
  -- SELF always = the submitter (p_profile_id), regardless of subject employee
  LEFT JOIN LATERAL (
  ...
  -- SUBJECT_EMPLOYEE = the employee this workflow is about (p_subject_employee_id)
  LEFT JOIN LATERAL (
    SELECT emp.name, emp.job_title
    FROM   employees emp
...
COMMENT ON FUNCTION get_workflow_participants(text, uuid, uuid) IS
  'Mig 646: SELF always resolves to submitter (p_profile_id). '
  'SUBJECT_EMPLOYEE resolves to subject employee (p_subject_employee_id). '
  'Fixes duplicate name in CC when admin submits on behalf of another employee.';
