-- =============================================================================
-- Migration 573: Rewrite fn_finalize_termination_execution
--
-- Supersedes migrations 564 and 572.
--
-- Key fixes vs mig 564:
--
-- STEP 1 — Terminated employee:
--   Added effective_from <= v_lwd guard on the UPDATE so the LWD+1 inactive
--   marker record is never touched.
--   Added DELETE for any spurious records starting after LWD+1.
--
-- STEP 2 — DR reassignments:
--   Replaced "fetch open record" logic with "find slice covering LWD" query.
--   Closes that slice at LWD, inserts an in-between slice from LWD+1 to
--   (first_future - 1) or 9999-12-31, then propagates the new manager to
--   ALL future slices (handles N future records, not just the open one).
--   Per-DR error isolation uses BEGIN...EXCEPTION...END (implicit PL/pgSQL
--   subtransaction) — no explicit SAVEPOINT/ROLLBACK TO needed.
--
-- Also reverses mig 572 (which wrongly closed Naveen's Jun-16 inactive marker)
-- in case it was already applied to the database.
-- =============================================================================

-- ── 0. Reverse migration 572 if it was applied ───────────────────────────────
DO $$
DECLARE
  v_emp_id uuid;
  v_rows   int;
BEGIN
  SELECT id INTO v_emp_id FROM employees WHERE employee_id = 'EMP010';
  ...
  WHERE  employee_id   = v_emp_id
    AND  effective_from = '2026-06-16'::date
    AND  effective_to   = '2026-06-15'::date;

  GET DIAGNOSTICS v_rows = ROW_COUNT;
  IF v_rows > 0 THEN
    RAISE NOTICE 'Reversed mig-572: re-opened Jun-16 inactive marker for EMP010.';
  END IF;
END;
$$;


-- ── 1. Rewrite fn_finalize_termination_execution ──────────────────────────────
...
COMMENT ON FUNCTION fn_finalize_termination_execution(uuid) IS
  'Mig 573: guards LWD+1 inactive marker, deletes spurious future records, '
  'finds covering slice, inserts in-between, propagates manager to all future slices.';
