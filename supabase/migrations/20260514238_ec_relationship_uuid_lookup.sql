-- =============================================================================
-- Migration 238: emergency_contacts.relationship — UUID → label lookup
--
-- emergency_contacts.relationship stores a UUID from the RELATIONSHIP_TYPE
-- picklist (same pattern as passport.country / identity country).
-- The field was returned as raw UUID in value; this patch resolves it to the
-- display label and marks input_type='select' for the section editor.
-- =============================================================================
...
DECLARE
  ...
  v_ec_relationship_lbl    text;   -- ← resolved label for ec.relationship
...
BEGIN
  ...
  -- Resolve ec.relationship UUID → display label
  ...
  -- ec.relationship is a UUID FK to the RELATIONSHIP_TYPE picklist.
  v_result := v_result || jsonb_build_object(
