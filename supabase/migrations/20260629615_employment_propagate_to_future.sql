-- ============================================================
-- Mig 615: employment propagation — p_propagate boolean
-- When inserting a new slice (non-correction cases), optionally
-- push explicitly-changed fields forward to all later slices.
-- ============================================================
...
BEGIN
  ...
  -- 5d. Open-ended slice → AMENDMENT; else GAP-FILL
  IF v_case IS NULL THEN
  ...
  -- ── 1b. Layer-B: fine-grained (correction=edit, else=create) ──────────────
  IF NOT v_is_system_path THEN
  ...
  ELSIF v_case = 'prepend' THEN
    INSERT INTO employee_employment (
      ...
      v_work_country,
      COALESCE(v_work_location, v_target.work_location),
  ...
  -- ── 10. Propagation — push explicitly-changed fields to all future slices ──
  --   Only when p_propagate = true AND the case is not a correction.
  --   "Explicitly changed" = the key exists in p_proposed_data AND is non-empty.
  --   We never overwrite hire_date or probation_end_date on future slices
  --   (those are point-in-time facts, not ongoing attributes).
  IF p_propagate AND v_case != 'correction' THEN
    UPDATE employee_employment
    SET
      designation = CASE
  ...
  -- ── 11. Sync employees table (head record) ────────────────────────────────
  -- Only sync when this insert affects the current/latest slice.
  -- Columns here must match what employees table actually has (no notice_period_days).
  UPDATE employees
  SET
    designation      = v_designation,
    job_title        = v_job_title,
...
COMMENT ON FUNCTION upsert_employment_info(uuid, jsonb, date, boolean) IS
  'Mig 615: added p_propagate boolean. When true and case != correction, pushes '
  'explicitly-provided fields forward to all future slices (effective_from > p_effective_from).';
