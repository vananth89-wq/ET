-- =============================================================================
-- Migration 406 — get_employee_hire_review: add Education sections
-- =============================================================================
-- Education records were missing from the hire review panel. Bank accounts and
-- dependents already loop over their respective set/item tables; education uses
-- a simpler flat table (employee_education) with no set wrapper.
--
-- Each active education row emits one "Education N" section so that
-- WorkflowReview.tsx can detect startsWith('Education') and hand off to
-- EducationPortlet (same portlet-dedup pattern used for bank/dependents).
-- =============================================================================
...
BEGIN
  ...
  -- ── Education sections ────────────────────────────────────────────────────
  -- One section per active row. WorkflowReview detects startsWith('Education')
  -- and renders a single EducationPortlet block (same portlet-dedup pattern as
  -- bank/dependents). Fields are read-only summary; the portlet handles editing.
  FOR v_edu_rec IN
...
COMMENT ON FUNCTION get_employee_hire_review(uuid) IS
  ...
  'Mig 406: Added Education sections (one per active employee_education row).';
