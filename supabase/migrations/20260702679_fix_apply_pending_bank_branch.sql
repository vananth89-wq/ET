-- =============================================================================
-- Mig 679: restore profile_bank apply branch in apply_profile_pending_change
--
-- ROOT CAUSE
-- ----------
-- Migs 677 and 678 replaced apply_profile_pending_change in full but carried
-- over the old NULL stub for profile_bank:
--   ELSIF v_module = 'profile_bank' THEN
--     NULL; -- handled by dedicated apply function (fn_apply_bank_set_transition)
-- The real wiring (fn_apply_bank_account_set_transition) was introduced in
-- mig 330 but was lost during the full-function rewrites.  As a result, bank
-- workflow approvals silently succeed but never materialise — the
-- employee_bank_account_set / items rows are never written.
--
-- FIX
-- ---
-- Replace only the profile_bank branch: NULL → real call to
-- fn_apply_bank_account_set_transition with the 20th-of-month approver cutoff
-- (bank_exceptions / admin / hr exempt) copied verbatim from mig 330.
-- =============================================================================
...
BEGIN
  ...
  ELSIF v_module = 'profile_bank' THEN
    -- MIG 679: restored from mig 330 — was erroneously set to NULL in migs 677/678.
    -- proposed_data shape (from submit_bank_account_set):
    ...
    DECLARE
      ...
      v_bank_set_id uuid;
    BEGIN
      v_bank_items := v_data->'items';
      IF jsonb_typeof(v_bank_items) <> 'array' THEN
        RAISE WARNING 'apply_profile_pending_change: profile_bank pending_change=% has no items[]; skipping. Row may predate set-snapshot model.', NEW.id;
      ELSE
        -- 20th-of-month approver cutoff
        ...
        THEN
          RAISE EXCEPTION
            ...
            'Today is %. Only bank_exceptions, admin, or hr roles may approve after this date.',
            TO_CHAR(CURRENT_DATE, 'DDth');
        END IF;

        v_eff_from    := COALESCE(NULLIF(v_data->>'effective_from','')::date, date_trunc('month', CURRENT_DATE)::date);
        v_bank_target := COALESCE(NULLIF(v_data->>'employee_id','')::uuid, v_emp_id);

        BEGIN
          v_bank_set_id := fn_apply_bank_account_set_transition(
            p_employee_id    => v_bank_target,
            p_effective_from => v_eff_from,
            p_items          => v_bank_items,
            p_actor          => NEW.submitted_by
          );
          RAISE NOTICE 'apply_profile_pending_change: applied bank account set pending_change=% set_id=% employee=%',
            NEW.id, v_bank_set_id, v_bank_target;
        EXCEPTION WHEN OTHERS THEN
          RAISE WARNING 'apply_profile_pending_change: fn_apply_bank_account_set_transition failed pending_change=% employee=% error=%',
            NEW.id, v_bank_target, SQLERRM;
        END;
      END IF;
    END;
...
COMMENT ON FUNCTION apply_profile_pending_change() IS
  ...
  'Mig 678: emergency_contact name uses NULLIF(...,'''') — Supabase JS strips null JSONB keys. '
  'Mig 679: profile_bank branch restored — calls fn_apply_bank_account_set_transition with 20th-of-month cutoff. '
  '         Was erroneously NULL in migs 677/678 full-function rewrites.';
