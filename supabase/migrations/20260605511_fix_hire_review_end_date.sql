-- =============================================================================
-- Migration 511 — Fix get_employee_hire_review: remove end_date from employees SELECT
--
-- Mig 487 dropped end_date from both employees and employee_employment.
-- Mig 494 rewrote get_employee_hire_review but still selected end_date from
-- employees, causing "column end_date does not exist" on every hire review load.
-- =============================================================================
...
COMMENT ON FUNCTION get_employee_hire_review(uuid) IS
  'Mig 511: removed end_date from employees SELECT (dropped by mig 487).';

DO $$
BEGIN
  RAISE NOTICE 'Migration 511: get_employee_hire_review — end_date column reference removed.';
END;
$$;
