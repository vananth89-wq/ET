-- =============================================================================
-- Migration 277: Add Bank Accounts section to get_employee_hire_review
--
-- The hire review RPC previously returned 7 sections:
--   Personal Info, Contact, Employment, Identity Document(s),
--   Passport, Address, Emergency Contact
--
-- This migration adds Section 8: Bank Account(s).
-- Bank accounts are always read-only in the hire review context — approvers
-- can view what the new hire submitted in AddEmployee Section 9, but changes
-- to bank accounts are made separately via upsert_bank_account.
--
-- One "Bank Account N" section is emitted per active account (effective_to =
-- '9999-12-31'). If no accounts exist a single placeholder section is shown
-- so the approver knows the section was skipped.
-- =============================================================================
...
BEGIN
  ...
  -- Option B: placeholder when no identity records
  IF v_id_idx = 0 THEN
    v_result := v_result || jsonb_build_object(
  ...
  -- One section per active account. Always rendered so approvers can see
  -- whether bank details were submitted or skipped in the hire form.
  FOR v_bank_rec IN
...
COMMENT ON FUNCTION get_employee_hire_review(uuid) IS
  ...
  'Mig 277: added Section 8 — Bank Account(s), read-only.';
...
BEGIN
  ...
  END IF;
  RAISE NOTICE 'Migration 277 verified: get_employee_hire_review now includes Bank Account section.';
END;
$$;

-- =============================================================================
-- END OF MIGRATION 277
-- =============================================================================
