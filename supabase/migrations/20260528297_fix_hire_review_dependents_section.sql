-- =============================================================================
-- Migration 297 — Fix get_employee_hire_review: restore correct picklist
--                 column names and add Dependents section properly
-- =============================================================================
--
-- BUG INTRODUCED BY MIGRATION 295
-- ─────────────────────────────────
-- Migration 295 added the Dependents section but accidentally broke the entire
-- function by using invalid column names throughout:
--
--   1. `full_name`   — column does not exist on picklist_values; correct: `value`
--   2. `date_of_birth` on employee_personal — column does not exist; correct: `dob`
--   3. `picklistId = '...' AND value = ...` lookups — column `picklistId` does not
--      exist; correct pattern: `id::text = <uuid_raw_value>`
--
-- PostgreSQL error 42703 ("column full_name does not exist") caused every call to
-- get_employee_hire_review to throw, making HireEnrichment silently return []
-- and the ApproverInbox right panel show no hire data at all.
--
-- FIX
-- ───
-- Restore the function based on the last known-good version (migration 277) and
-- append the Dependents section using the same `value` / `id::text` pattern.
-- =============================================================================
...
BEGIN
  ...
  -- employee_personal column is `dob` (not `date_of_birth`)
  ...
  -- Placeholder when no identity records
  IF v_id_idx = 0 THEN
    v_result := v_result || jsonb_build_object(
  ...
  -- relationship_type stores a picklist UUID (ref_id from DEPENDENT_RELATIONSHIP_TYPE)
  -- — looked up with the same id::text pattern used throughout this function.
  FOR v_dep_rec IN
  ...
  LOOP
    ...
    IF v_dep_rec.relationship_type IS NOT NULL THEN
      -- relationship_type stores the picklist ref_id code (e.g. "DR002"), not a UUID.
      -- Join to picklists to scope by DEPENDENT_RELATIONSHIP_TYPE and look up by ref_id.
  ...
  -- Placeholder when no dependents were submitted (this section is optional)
  IF v_dep_idx = 0 THEN
    v_result := v_result || jsonb_build_object(
      'section', 'Dependent',
      'fields', jsonb_build_array(
...
COMMENT ON FUNCTION get_employee_hire_review(uuid) IS
  ...
  'Mig 297: fixed full_name→value, date_of_birth→dob, picklistId lookups (all '
  'broken in mig 295). Added Section 9 — Dependent(s), read-only, correctly.';
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'ABORT: get_employee_hire_review not found after migration 297.';
  END IF;
  RAISE NOTICE 'Migration 297 verified: get_employee_hire_review restored with correct picklist column names and Dependents section.';
END;
$$;
...
--
-- Root cause summary:
--   Migration 295 rewrote get_employee_hire_review using `full_name` (does not
--   exist on picklist_values), `date_of_birth` (does not exist on
--   employee_personal; correct: `dob`), and `picklistId = '...' AND value = ...`
--   lookups (correct: `id::text = <uuid>`). PostgreSQL error 42703 caused every
--   RPC call to throw, making HireEnrichment return [] silently and the
--   ApproverInbox right panel show no hire data sections.
-- =============================================================================
