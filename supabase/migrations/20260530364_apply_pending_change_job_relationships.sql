-- =============================================================================
-- Migration 364 — Wire profile_job_relationships into workflow apply path
--
-- TWO CHANGES:
--
-- 1. submit_change_request — add profile_job_relationships snapshot branch
--    Snapshots the current active set + items into current_data so approvers
--    see a before/after diff in the workflow review screen.
--
-- 2. apply_profile_pending_change — add profile_job_relationships apply branch
--    When a profile_job_relationships request is approved, calls
--    upsert_job_relationship_set() which handles the full effective-dated
--    close-then-insert and mirror sync. Afterwards queues notifications.
--    Also registers profile_job_relationships in the handled module list.
--
-- Design spec: docs/job-relationships-design.md §11 Phase 6
-- Template:    mig 354 (profile_employment apply branch)
-- =============================================================================


-- =============================================================================
-- 1. submit_change_request — add profile_job_relationships snapshot
-- =============================================================================
-- Full function recreated. Only the CASE block gains the new branch.
-- All other branches are unchanged from mig 354 (last authoritative edit).
-- =============================================================================
...
BEGIN
  ...
  IF v_emp_id IS NOT NULL AND p_action = 'update' THEN
    ...
    CASE p_module_code
      ...
      -- ── NEW: profile_job_relationships snapshot ──────────────────────────
      WHEN 'profile_job_relationships' THEN
        -- Snapshot the active set + its items as a JSONB object for the diff panel
...
COMMENT ON FUNCTION submit_change_request(text, uuid, jsonb, text, text) IS
  ...
  'Mig 364: added profile_job_relationships snapshot branch (active set + items).';
...
-- =============================================================================
-- 2. apply_profile_pending_change — add profile_job_relationships branch
-- =============================================================================
-- Full trigger function recreated. Only the ELSIF chain gains the new branch.
-- All other branches unchanged from mig 354.
-- =============================================================================
...
BEGIN
  ...
  ELSIF v_module = 'profile_job_relationships' THEN
    ...
    -- Capture the current open set_id BEFORE upsert (for notification diff)
    ...
    IF NOT (v_result->>'ok')::boolean THEN
      RAISE WARNING
        ...
        v_emp_id, v_result->>'error';
    ELSE
      -- Queue assigned/removed notifications for the newly applied set
      BEGIN
...
COMMENT ON FUNCTION apply_profile_pending_change() IS
  ...
  'profile_contact, profile_address, profile_passport, profile_identification, '
  'profile_emergency_contact, profile_bank, profile_dependents. '
  'Mig 364: added profile_job_relationships branch (calls upsert_job_relationship_set '
  'then fn_queue_job_relationship_notifications).';


-- =============================================================================
-- 3. Register profile_job_relationships in module_codes (if table exists)
-- =============================================================================
...
BEGIN
  IF EXISTS (
    ...
    VALUES ('profile_job_relationships', 'Job Relationships')
...
SELECT proname,
       prosrc LIKE '%profile_job_relationships%' AS has_jr_branch
FROM   pg_proc
WHERE  proname IN ('submit_change_request', 'apply_profile_pending_change')
ORDER  BY proname;

-- =============================================================================
-- END OF MIGRATION 364
-- =============================================================================
