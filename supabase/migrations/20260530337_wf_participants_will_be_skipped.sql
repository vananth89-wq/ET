-- =============================================================================
-- Migration 337 — get_workflow_participants: add willBeSkipped flag
-- =============================================================================
--
-- Adds a boolean `willBeSkipped` field to each step in the response.
-- true  when: approver_type IN ('MANAGER','DEPT_HEAD')
--             AND p_profile_id was supplied (so we can evaluate)
--             AND the submitter has no manager / dept head resolvable
-- false in all other cases.
--
-- This lets WorkflowSubmitModal render a "Skipped" visual state for steps
-- that will be auto-skipped by wf_submit (mig 336) rather than showing
-- them as normal pending steps.
-- =============================================================================
...
BEGIN
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      -- ── NEW mig 337 ────────────────────────────────────────────────────────
      -- willBeSkipped: true when wf_submit will auto-skip this step because
      -- no MANAGER / DEPT_HEAD is found in the org structure for this submitter.
      -- Only evaluated when p_profile_id is provided; otherwise always false.
      'willBeSkipped',
        CASE
          WHEN ws.approver_type IN ('MANAGER', 'DEPT_HEAD')
               AND p_profile_id IS NOT NULL
               AND mgr_emp.name IS NULL
            THEN true
          ELSE false
        END,
...
COMMENT ON FUNCTION get_workflow_participants(text, uuid) IS
  ...
  'willBeSkipped (mig 337): true for MANAGER/DEPT_HEAD steps where the submitter '
  'has no resolvable manager — wf_submit (mig 336) will auto-skip these steps. '
  'ROLE steps (mig 205): roleMembers array lists all active role holders. '
  'approvalMode: ALL_OF = all must approve; null = first to approve wins.';
