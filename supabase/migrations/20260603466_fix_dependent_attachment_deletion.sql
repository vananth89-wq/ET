-- Migration: fix dependent attachment deletion
-- Problem: fn_apply_dependent_set_transition only inserts new attachments;
--   it never removes attachments that the user deleted. This caused removed
--   attachments to reappear after saving.
-- Fix: after inserting new rows, DELETE any existing attachment rows for the
--   dependent_code that were NOT included in the submitted attachments list.
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- 5. Reconcile attachments for this dependent_code.
    --    When the caller provides an 'attachments' array we treat it as the
    --    full desired state: insert rows that are new, delete rows that were
    --    removed by the user (present in DB but absent from the submitted list).
    IF jsonb_typeof(v_item->'attachments') = 'array' THEN

      -- 5a. Insert any new attachment rows not already present.
      ...
      -- 5b. Delete attachment rows that the user removed (in DB but not in
      --     the submitted list). An empty array means all attachments deleted.
      DELETE FROM employee_dependent_attachments
      WHERE dependent_code = v_dep_code
        AND (
          -- No file_paths submitted at all (empty array)
          NOT EXISTS (
            SELECT 1 FROM jsonb_array_elements(v_item->'attachments') att
            WHERE (att->>'file_path') IS NOT NULL AND (att->>'file_path') <> ''
          )
          OR
          -- This row's file_path is absent from the submitted list
          file_path NOT IN (
...
COMMENT ON FUNCTION fn_apply_dependent_set_transition(UUID, DATE, JSONB, UUID) IS
  ...
  'items, auto-assigns dependent_code to NEW items, reconciles attachment rows '
  '(inserts new, deletes removed). Called from submit_dependent_set (PATH A) '
  'and from apply_profile_pending_change (PATH B). '
  'Trusts its caller for access checks. Fixed in mig 454.';
