-- =============================================================================
-- Migration 416 — Fix education WHEN clause: remove field_of_study
--
-- Mig 415 was pushed with field_of_study still in the education export query.
-- That column was dropped in mig 405. This migration patches the education
-- WHEN clause only (all other 15 clauses unchanged via CREATE OR REPLACE).
--
-- Verified columns in employee_education after mig 405:
--   education_level, degree, institution, start_date, end_date,
--   completion_status, grade_or_gpa, is_highest_qualification,
--   is_active, inactive_at, created_by, updated_by, created_at, updated_at, id
--
-- Predecessor: mig 415 (education clause restored but contained dropped column)
-- =============================================================================
...
BEGIN
  ...
  CASE p_template_code
    ...
    -- =========================================================================
    -- 16. education  (field_of_study removed in mig 405; sys meta added mig 416)
    --
    -- Columns verified against employee_education after mig 405:
    --   education_level, degree, institution, start_date, end_date,
    --   completion_status, grade_or_gpa, is_highest_qualification,
    --   is_active, inactive_at, created_at, updated_at, id
    -- =========================================================================
    WHEN 'education' THEN
      IF p_mode = 'history' THEN
        RETURN QUERY SELECT to_jsonb(r) FROM (
...
COMMENT ON FUNCTION bulk_export IS
  'Mig 416: all 16 templates. Education field_of_study removed (dropped in mig 405).';
