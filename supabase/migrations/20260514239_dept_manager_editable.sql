-- =============================================================================
-- Migration 239: Make Department and Manager editable in hire review
--
-- Department (emp.dept_id)  → input_type 'dept_select'  (options from departments table)
-- Manager    (emp.manager_id) → input_type 'emp_select'  (options from employees table)
-- Both store UUID values directly on the employees row.
--
-- Also adds WHEN cases to update_hire_field for these two fields.
-- =============================================================================

-- ── 1. update_hire_field: add dept_id and manager_id cases ───────────────────
...
BEGIN
  CASE p_field_key
    ...
    WHEN 'emp.designation' THEN
      UPDATE employees SET designation = NULLIF(p_new_value,'') WHERE id = p_employee_id;
    WHEN 'emp.work_country' THEN
      UPDATE employees SET work_country = NULLIF(p_new_value,'') WHERE id = p_employee_id;
    WHEN 'emp.work_location' THEN
      UPDATE employees SET work_location = NULLIF(p_new_value,'') WHERE id = p_employee_id;
    WHEN 'emp.dept_id' THEN
      UPDATE employees SET dept_id = NULLIF(p_new_value,'')::uuid WHERE id = p_employee_id;
    WHEN 'emp.manager_id' THEN
      UPDATE employees SET manager_id = NULLIF(p_new_value,'')::uuid WHERE id = p_employee_id;
    ...
    WHEN 'personal.marital_status' THEN
      ...
      VALUES (p_employee_id, NULLIF(p_new_value,''))
    ...
    WHEN 'passport.country' THEN
      INSERT INTO passports (employee_id, country)
      VALUES (p_employee_id, NULLIF(p_new_value,''))
      ON CONFLICT (employee_id) DO UPDATE SET country = EXCLUDED.country;
    WHEN 'passport.passport_number' THEN
      INSERT INTO passports (employee_id, passport_number)
      VALUES (p_employee_id, p_new_value)
      ON CONFLICT (employee_id) DO UPDATE SET passport_number = EXCLUDED.passport_number;
    WHEN 'passport.issue_date' THEN
      INSERT INTO passports (employee_id, issue_date)
      VALUES (p_employee_id, NULLIF(p_new_value,'')::date)
      ON CONFLICT (employee_id) DO UPDATE SET issue_date = EXCLUDED.issue_date;
    WHEN 'passport.expiry_date' THEN
      INSERT INTO passports (employee_id, expiry_date)
      VALUES (p_employee_id, NULLIF(p_new_value,'')::date)
      ON CONFLICT (employee_id) DO UPDATE SET expiry_date = EXCLUDED.expiry_date;

    -- ── emergency_contacts ────────────────────────────────────────────────────
    WHEN 'ec.name' THEN
      INSERT INTO emergency_contacts (employee_id, name)
      VALUES (p_employee_id, p_new_value)
      ON CONFLICT (employee_id) DO UPDATE SET name = EXCLUDED.name;
    WHEN 'ec.relationship' THEN
      INSERT INTO emergency_contacts (employee_id, relationship)
      VALUES (p_employee_id, NULLIF(p_new_value,''))
      ON CONFLICT (employee_id) DO UPDATE SET relationship = EXCLUDED.relationship;
    WHEN 'ec.phone' THEN
      INSERT INTO emergency_contacts (employee_id, phone)
      VALUES (p_employee_id, p_new_value)
      ON CONFLICT (employee_id) DO UPDATE SET phone = EXCLUDED.phone;
    WHEN 'ec.alt_phone' THEN
      INSERT INTO emergency_contacts (employee_id, alt_phone)
      VALUES (p_employee_id, p_new_value)
      ON CONFLICT (employee_id) DO UPDATE SET alt_phone = EXCLUDED.alt_phone;
    WHEN 'ec.email' THEN
      INSERT INTO emergency_contacts (employee_id, email)
      VALUES (p_employee_id, p_new_value)
      ON CONFLICT (employee_id) DO UPDATE SET email = EXCLUDED.email;

    -- ── identity_records (keyed by id.<uuid>.<column>) ────────────────────────
    ELSE
      IF p_field_key ~ '^id\.[0-9a-f-]{36}\.' THEN
        DECLARE
          v_parts   text[]  := string_to_array(p_field_key, '.');
          v_rec_id  uuid    := v_parts[2]::uuid;
          v_col     text    := v_parts[3];
        BEGIN
          CASE v_col
            WHEN 'country'     THEN UPDATE identity_records SET country     = NULLIF(p_new_value,'') WHERE id = v_rec_id AND employee_id = p_employee_id;
            WHEN 'id_type'     THEN UPDATE identity_records SET id_type     = NULLIF(p_new_value,'') WHERE id = v_rec_id AND employee_id = p_employee_id;
            WHEN 'record_type' THEN UPDATE identity_records SET record_type = p_new_value            WHERE id = v_rec_id AND employee_id = p_employee_id;
            WHEN 'id_number'   THEN UPDATE identity_records SET id_number   = p_new_value            WHERE id = v_rec_id AND employee_id = p_employee_id;
            WHEN 'expiry'      THEN UPDATE identity_records SET expiry      = NULLIF(p_new_value,'')::date WHERE id = v_rec_id AND employee_id = p_employee_id;
            ELSE NULL;
          END CASE;
        END;
      END IF;
...
-- ── 2. get_employee_hire_review: flip dept_id and manager_id to editable ─────
...
BEGIN
  ...
  v_result := v_result || jsonb_build_object(
    ...
    'fields', jsonb_build_array(
      ...
      jsonb_build_object('label','Department',    'value',COALESCE(v_dept,                               '—'),'raw_value',v_dept_id::text,             'key','emp.dept_id',                  'editable',true, 'input_type','dept_select'),
      jsonb_build_object('label','Manager',       'value',COALESCE(v_manager,                            '—'),'raw_value',v_manager_id::text,          'key','emp.manager_id',               'editable',true, 'input_type','emp_select'),
  ...
  v_result := v_result || jsonb_build_object(
    'section', 'Passport',
    'fields', jsonb_build_array(
      jsonb_build_object('label','Country',         'value',COALESCE(v_passport_country_lbl,     '—'),'raw_value',v_passport.country,         'key','passport.country',         'editable',true,'input_type','select'),
  ...
  v_result := v_result || jsonb_build_object(
    ...
    'fields', jsonb_build_array(
      ...
      jsonb_build_object('label','Relationship', 'value',COALESCE(v_ec_relationship_lbl,  '—'),'raw_value',v_ec.relationship,'key','ec.relationship','editable',true,'input_type','select'),
