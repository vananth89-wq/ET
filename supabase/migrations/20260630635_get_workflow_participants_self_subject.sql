-- =============================================================================
-- Mig 635: get_workflow_participants — fix SELF step resolves subject, not admin
--
-- Bug: SELF (Submitter/CC) step always resolves via p_profile_id (the logged-in
-- user). When an admin submits on behalf of an employee, p_profile_id = admin →
-- CC shows admin's name instead of the subject employee's name.
--
-- Fix: when p_subject_employee_id is supplied, resolve SELF from the subject
-- employee directly. For self-service (p_subject_employee_id IS NULL), fall back
-- to p_profile_id → same behaviour as before.
-- =============================================================================
...
BEGIN
    ...
    jsonb_build_object(
      ...
      'resolvedName',
        CASE ws.approver_type
          ...
          WHEN 'SELF'          THEN COALESCE(self_emp_subject.name, self_emp.name, 'You')
          ELSE                      ws.name
        END,
      ...
      'hasResolvedPerson',
        CASE ws.approver_type
          ...
          WHEN 'SELF'          THEN (COALESCE(self_emp_subject.name, self_emp.name) IS NOT NULL)
          ELSE                      false
        END,
  ...
  -- MANAGER / DEPT_HEAD: self-service (no subject override)
  LEFT JOIN LATERAL (
    SELECT mgr.name, mgr.job_title, pv.value AS designation_label
    FROM   profiles sp
  ...
  -- SELF: admin-on-behalf → resolve from subject employee directly
  LEFT JOIN LATERAL (
    SELECT emp.name, emp.job_title
    FROM   employees emp
    WHERE  emp.id = p_subject_employee_id
    LIMIT  1
  ) self_emp_subject ON ws.approver_type = 'SELF'
                    AND p_subject_employee_id IS NOT NULL

  -- SELF: self-service → resolve from submitter's profile
  LEFT JOIN LATERAL (
    SELECT emp.name, emp.job_title
    FROM   profiles sp
...
COMMENT ON FUNCTION get_workflow_participants(text, uuid, uuid) IS
  'Mig 635: SELF step now resolves subject employee when p_subject_employee_id '
  'is supplied (admin-on-behalf). Self-service unchanged.';
