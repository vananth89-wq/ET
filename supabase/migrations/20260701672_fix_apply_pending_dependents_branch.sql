-- Migration 672 — Restore profile_dependents branch in apply_profile_pending_change
-- ──────────────────────────────────────────────────────────────────────────────
-- ROOT CAUSE
-- The mig 321 wiring of fn_apply_dependent_set_transition was silently dropped
-- in every subsequent full rewrite of this trigger (migs 661, 667, 670).
-- Mig 670 (the current live version) has:
--   ELSIF v_module IN ('profile_bank', 'profile_dependents') THEN NULL;
-- which means dependent set approvals are a no-op — the approved data is never
-- written back, so the Dependents tab stays empty after approval.
--
-- FIX
-- Split the combined NULL branch:
--   profile_dependents → call fn_apply_dependent_set_transition
--   profile_bank       → keep NULL (handled by fn_apply_bank_set_transition)
-- Full function rewrite to update the comment/version stamp.
...
COMMENT ON FUNCTION apply_profile_pending_change() IS
  ...
  'profile_bank branch kept as NULL (fn_apply_bank_set_transition handles it separately).';
