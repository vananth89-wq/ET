-- Migration 570 — Add p_subject_employee_id to submit_change_request
-- ──────────────────────────────────────────────────────────────────
-- When HR submits a profile change on behalf of another employee (e.g. via
-- the admin MyProfile view), workflow_instances.subject_profile_id must
-- reflect the subject employee — not the HR actor. Without this,
-- vw_wf_operations shows the HR user's name in the Employee column, and
-- the wf_submit MANAGER resolver picks the HR user's manager instead of
-- the subject employee's manager.
--
-- Change: add optional p_subject_employee_id uuid DEFAULT NULL.
-- When supplied, use it for:
--   1. current-data snapshot (read subject's satellite rows, not caller's)
--   2. wf_submit p_subject_employee_id (stamps subject_profile_id on instance)
...
CREATE OR REPLACE FUNCTION submit_change_request(
  ...
  p_subject_employee_id  uuid    DEFAULT NULL   -- mig 570: HR-on-behalf-of support
)
RETURNS jsonb
LANGUAGE plpgsql
VOLATILE
...
BEGIN
  ...
  -- ── Resolve workflow via assignment table ────────────────────────────────────
  ...
  v_instance_id := wf_submit(
    ...
    p_subject_employee_id => p_subject_employee_id   -- mig 570: stamp subject on instance
  );
...
COMMENT ON FUNCTION submit_change_request(text, uuid, jsonb, text, text, uuid) IS
  ...
  'Mig 570: p_subject_employee_id added — when HR submits on behalf of another '
  'employee, the subject''s data is snapshotted and wf_submit stamps '
  'subject_profile_id on the workflow instance so MANAGER steps resolve '
  'the subject''s manager, not the HR actor''s manager.';
