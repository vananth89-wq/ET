-- =============================================================================
-- Migration 713 — Add work_schedule_id / holiday_calendar_id support to
--                 upsert_employment_info and get_employment_info_history
--
-- Context:
--   Mig 703 added work_schedule_id and holiday_calendar_id columns to
--   employee_employment. This migration teaches the upsert function to read
--   those fields from p_proposed_data and persist them, and updates the
--   history RPC to return them.
--
-- Safe to run multiple times (CREATE OR REPLACE).
-- =============================================================================

-- ── 1. Update upsert_employment_info (4-arg form from mig 616) ───────────────
...
BEGIN
  ...
  ELSIF v_case = 'prepend' THEN
    INSERT INTO employee_employment (
      ...
      work_schedule_id, holiday_calendar_id,
      ...
      COALESCE(v_work_schedule_id, v_target.work_schedule_id),
      COALESCE(v_holiday_cal_id,   v_target.holiday_calendar_id),
  ...
  -- (work_schedule_id / holiday_calendar_id are not mirrored to employees table)
  UPDATE employees
  SET
    designation      = v_designation,
    job_title        = v_job_title,
...
COMMENT ON FUNCTION upsert_employment_info(uuid, jsonb, date, boolean) IS
  'Mig 713: adds work_schedule_id and holiday_calendar_id handling. '
  'All INSERT/UPDATE cases persist scheduling fields from p_proposed_data. '
  'COALESCE fallback preserves existing scheduling assignment.';


-- ── 2. Update get_employment_info_history to include scheduling fields ────────
...
COMMENT ON FUNCTION get_employment_info_history(uuid) IS
  'Mig 713: added work_schedule_id and holiday_calendar_id to returned fields.';
...
BEGIN
  ...
  RAISE NOTICE 'Migration 713: scheduling fields added to upsert_employment_info and get_employment_info_history.';
END;
$$;
