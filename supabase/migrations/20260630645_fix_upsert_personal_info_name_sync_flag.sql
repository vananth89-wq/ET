-- =============================================================================
-- Mig 645: fix upsert_personal_info — set prowess.allow_name_sync before
--          updating employees.name
--
-- fn_guard_employee_name_sync() blocks direct name changes on Active employees
-- unless the session variable prowess.allow_name_sync = 'true' is set.
-- upsert_personal_info step 8 updates employees.name without setting the flag,
-- so the guard trigger raises an exception and the whole function fails.
--
-- Fix: SET LOCAL prowess.allow_name_sync = 'true' immediately before the
-- employees UPDATE. SET LOCAL resets automatically at transaction end.
-- =============================================================================
...
BEGIN
  ...
  -- No session = called from a SECURITY DEFINER trigger
  ...
  -- SET LOCAL allows fn_guard_employee_name_sync to pass through for this transaction.
  IF p_effective_from <= CURRENT_DATE AND v_computed_name IS NOT NULL THEN
    PERFORM set_config('prowess.allow_name_sync', 'true', true);  -- true = LOCAL (resets at txn end)
    UPDATE employees
...
COMMENT ON FUNCTION upsert_personal_info(uuid, jsonb, date, boolean) IS
  'Mig 645: set_config(prowess.allow_name_sync, true, true) before updating '
  'employees.name so fn_guard_employee_name_sync() passes through.';
