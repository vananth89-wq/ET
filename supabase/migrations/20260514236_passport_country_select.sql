-- =============================================================================
-- Migration 236: passport.country — change input_type to 'select'
--
-- passports.country stores a plain country-name string (same as nationality),
-- not a picklist UUID.  The field was incorrectly marked input_type='text' in
-- migration 235.  This patch switches it to 'select' so the frontend renders
-- the COUNTRIES static dropdown and the stored value round-trips correctly.
-- =============================================================================
...
BEGIN
  ...
  -- passport.country stores a plain country-name string (same as nationality),
  -- so input_type='select' with the COUNTRIES static list on the frontend.
  v_result := v_result || jsonb_build_object(
    'section', 'Passport',
    'fields', jsonb_build_array(
      jsonb_build_object('label','Country',         'value',COALESCE(v_passport.country,         '—'),'raw_value',v_passport.country,         'key','passport.country',         'editable',true,'input_type','select'),
...
COMMENT ON FUNCTION get_employee_hire_review(uuid) IS
  ...
  'passport.country is input_type=select (COUNTRIES static list, text values). '
  'id.*.country is input_type=select (ID_COUNTRY picklist, UUID values).';
