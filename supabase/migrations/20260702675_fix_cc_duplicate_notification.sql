-- =============================================================================
-- Migration 675: Fix duplicate CC notifications when submitter = subject employee
--
-- PROBLEM
-- ───────
-- When an employee submits their own Personal Info workflow (submitter IS the
-- subject employee), templates with multiple CC steps (e.g. Step 3 = SELF and
-- Step 4 = SUBJECT_EMPLOYEE) resolve both steps to the same person.
-- wf_advance_instance fires once per CC step → the same profile receives two
-- identical "you were CC'd" notifications.
--
-- Frontend dedup (mig 675 companion): useWorkflowParticipants.ts already
-- deduplicates the CC display in WorkflowSubmitModal using a Set of resolvedName.
-- But the backend still creates two CC tasks and calls wf_queue_notification twice.
--
-- FIX
-- ───
-- In wf_advance_instance (Path C), before calling wf_queue_notification for a
-- CC step, check whether this instance already has a completed CC task assigned
-- to the same profile. If yes: mark the new task as approved with a
-- 'CC — duplicate skipped' note, log it, and advance — without sending another
-- notification.
--
-- The check uses workflow_tasks (assigned_to + is_cc + status='approved') rather
-- than workflow_action_log.actor_id because actor_id is auth.uid() (the person
-- triggering advance), not the CC recipient.
-- =============================================================================
...
BEGIN
  ...
  -- ── CC step: deduplicate notification before sending ──────────────────────
  -- FIX (mig 675): If this CC step's resolved approver already received a CC
  -- notification on this instance (i.e. a prior CC step resolved to the same
  -- profile — happens when submitter = subject employee), skip the duplicate
  -- notification. The task is still auto-completed so the instance can advance.
  IF v_next_step.is_cc THEN
    IF EXISTS (
      SELECT 1
      FROM   workflow_tasks   wt2
      JOIN   workflow_steps   ws2 ON ws2.id = wt2.step_id
      WHERE  wt2.instance_id = p_instance_id
        AND  wt2.assigned_to = v_approver_id
        AND  wt2.status      = 'approved'
        AND  ws2.is_cc       = true
        AND  wt2.id         <> v_new_task_id
    ) THEN
      -- Duplicate CC recipient — auto-complete silently without re-notifying
      UPDATE workflow_tasks
      SET    status   = 'approved',
             acted_at = now(),
             notes    = 'CC — duplicate skipped (same profile already notified)'
      ...
      VALUES
        (p_instance_id, v_new_task_id, auth.uid(), 'cc_notified',
         v_next_step.step_order,
         'CC step — duplicate skipped, profile already notified on this instance');
...
COMMENT ON FUNCTION wf_advance_instance(uuid) IS
  ...
  'Mig 675: CC deduplication — if this instance already has a completed CC task '
  'for the same profile (e.g. submitter = subject employee, SELF + SUBJECT_EMPLOYEE '
  'both resolve to same person), skip the duplicate notification and auto-complete. '
  'Path A — Multi-approver: approval_mode IS NOT NULL. '
