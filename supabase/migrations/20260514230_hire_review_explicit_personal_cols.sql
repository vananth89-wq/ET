-- =============================================================================
-- Migration 230: get_employee_hire_review — explicit column SELECT for
--                employee_personal (drop %ROWTYPE for that table)
--
-- PROBLEM
-- ───────
-- Migrations 228 and 229 both fail to resolve marital_status because
-- `v_personal employee_personal%ROWTYPE` binds columns by position at
-- parse time.  If marital_status was added to employee_personal after the
-- table was first created, PL/pgSQL's column-position mapping is stale and
-- v_personal.marital_status silently reads as NULL even though the DB row
-- contains the value.
--
-- The same misalignment does NOT affect nationality/gender/dob because those
-- columns appear earlier in the physical layout and their positions match.
--
-- FIX
-- ───
-- Remove the employee_personal%ROWTYPE variable entirely.  Instead, SELECT
-- each required column explicitly into individual scalar variables.  Explicit
-- column lists are immune to physical column-order issues.
-- =============================================================================
...
DECLARE
  ...
  -- employee_personal — explicit scalars (avoids %ROWTYPE column-order issues)
  v_nationality    text;
  v_marital_raw    text;
  v_gender         text;
  v_dob            date;

  -- Other satellite rows (single-row tables, %ROWTYPE is fine there)
...
BEGIN
  ...
  v_result := v_result || jsonb_build_object(
    ...
    'fields', jsonb_build_array(
      ...
      jsonb_build_object('label', 'Nationality',    'value', COALESCE(v_nationality,   '—')),
      jsonb_build_object('label', 'Marital Status', 'value', COALESCE(v_marital_label, '—')),
      jsonb_build_object('label', 'Gender',         'value', COALESCE(v_gender,        '—')),
      jsonb_build_object('label', 'Date of Birth',  'value', COALESCE(v_dob::text,     '—')),
...
COMMENT ON FUNCTION get_employee_hire_review(uuid) IS
  'Returns all employee sections as JSONB [{section, fields:[{label,value}]}] for WorkflowReview. '
  'employee_personal fields are fetched with an explicit column SELECT (not %ROWTYPE) to avoid '
  'PL/pgSQL column-position misalignment when columns were added after initial table creation.';
