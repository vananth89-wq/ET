-- Migration 667 — Fix apply_profile_pending_change: satellite lookup by employee_id
-- ─────────────────────────────────────────────────────────────────────────────
-- ROOT CAUSE
-- ──────────
-- For passport, identification, and emergency_contact the trigger checked:
--   EXISTS (SELECT 1 FROM passports WHERE id = NEW.record_id)
-- When record_id = employees.id (which is what MyProfile submits for these
-- modules), this check is false → trigger falls through to INSERT → unique
-- constraint violation (passports_employee_id_key, etc.).
--
-- FIX (same pattern as address in mig 661/662):
-- 1. Look up the satellite row by employee_id first.
-- 2. Fall back to record_id as satellite UUID only if step 1 found nothing.
...
BEGIN
  ...
  ELSIF v_module = 'profile_passport' THEN
    -- Look up by employee_id first (record_id may be employees.id, not passport UUID)
  ...
  ELSIF v_module = 'profile_identification' THEN
    -- Look up by employee_id first (no unique constraint per row, so take latest active)
...
COMMENT ON FUNCTION apply_profile_pending_change() IS
  ...
  'Mig 667: passport, identification, emergency_contact now look up satellite row by employee_id first, '
  'just like address. Fixes "duplicate key violates passports_employee_id_key" when record_id = employees.id.';
