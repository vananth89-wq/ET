-- =============================================================================
-- Migration 227: get_employee_hire_review — resolve picklist UUIDs to labels
--
-- PROBLEM
-- ───────
-- Several fields on employees and employee_personal are stored as picklist
-- UUIDs (selected from a <select> backed by picklist_values):
--
--   employees.designation   → UUID stored, need label
--   employees.work_country  → UUID stored, need label
--   employees.work_location → UUID stored, need label
--   employee_personal.marital_status → UUID stored, need label
--
-- Other fields (nationality, gender, name, emails, mobile) are plain-text —
-- no resolution needed.
--
-- FIX
-- ───
-- For each UUID field, do a subquery against picklist_values to resolve the
-- label. Falls back to the raw value if not found (i.e. already plain text).
-- =============================================================================
...
DECLARE
  ...
  v_designation    text;    -- may be a picklist UUID
  v_dept_id        uuid;
  v_manager_id     uuid;
  v_hire_date      date;
  v_end_date       date;
  v_work_country   text;    -- may be a picklist UUID
  v_work_location  text;    -- may be a picklist UUID
  v_base_curr_id   uuid;
  v_status         text;
...
BEGIN
  ...
  -- ── Resolve picklist UUIDs → display labels ───────────────────────────────
  -- Each field may be a plain-text value OR a UUID key into picklist_values.
  -- The subquery returns NULL if no match; COALESCE then falls back to the
  -- raw value (which is fine if it was plain text to begin with).
  ...
  -- Resolve marital_status UUID → label (stored as picklist UUID in employee_personal)
...
COMMENT ON FUNCTION get_employee_hire_review(uuid) IS
  ...
  'Resolves picklist UUIDs (designation, work_country, work_location, marital_status) via picklist_values. '
  'Queries satellite tables matching the live DB schema.';
