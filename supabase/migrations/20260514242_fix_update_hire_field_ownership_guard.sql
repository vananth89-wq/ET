-- =============================================================================
-- Migration 242: fix update_hire_field — replace role_permissions join with user_can()
--
-- Migration 241 added a dual-path ownership guard but referenced the old
-- role_permissions table which was dropped in migration 146.  This migration
-- rewrites PATH B to call user_can('hire_employee', 'edit', NULL) — the same
-- function used by all other RLS policies in the system.
--
-- PATH A (submitter) is unchanged.
-- PATH B (approver)  now calls user_can() directly.
-- =============================================================================
...
COMMENT ON FUNCTION update_hire_field(uuid, text, text) IS
  ...
  'submitter (initiator editing a sent-back record) or pass user_can(hire_employee, edit, NULL) '
  '(approver mid-flight edit). Only operates on Pending employees.';
