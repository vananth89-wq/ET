-- =============================================================================
-- Migration 414 — Restore education WHEN clause in bulk_export + add sys meta
--
-- PROBLEM: Mig 413 used CREATE OR REPLACE on bulk_export with only 15 WHEN
-- clauses, overwriting mig 398's 16-clause version and losing 'education'.
-- Result: "Export failed: Unknown template_code: education."
--
-- FIX: CREATE OR REPLACE with all 16 WHEN clauses (15 from mig 413 verbatim +
-- education restored from mig 398, upgraded with id/Created At/Updated At).
--
-- Also updates education schema_definition to include system metadata columns.
--
-- Predecessor: mig 413 (15-template sys meta), mig 398 (education export)
-- =============================================================================
...
COMMENT ON FUNCTION bulk_export IS
  'Mig 414: all 16 templates with full system metadata support. '
  'Education clause restored after mig 413 accidentally dropped it.';
...
-- =============================================================================
-- Update education schema_definition to include system metadata columns
-- =============================================================================
...
UPDATE bulk_template_registry SET schema_definition = jsonb_build_object(
  'columns', (SELECT jsonb_agg(col ORDER BY ord) FROM (VALUES
    (jsonb_build_object('name','Employee Code *',          'data_type','code_employee',               'mandatory',true, 'user_fillable',true,'description','Existing employee code e.g. EMP001'),  1),
    (jsonb_build_object('name','Education Level Code *',   'data_type','code_picklist:EDUCATION_LEVEL','mandatory',true, 'user_fillable',true,'description','Picklist ref_id e.g. EDU03'),          2),
    (jsonb_build_object('name','Degree *',                 'data_type','text',                        'mandatory',true, 'user_fillable',true,'description','e.g. B.Tech in Computer Science'),      3),
    (jsonb_build_object('name','Institution *',            'data_type','text',                        'mandatory',true, 'user_fillable',true,'description','e.g. Anna University'),                  4),
    (jsonb_build_object('name','Start Date *',             'data_type','date_mmddyyyy',               'mandatory',true, 'user_fillable',true),                                                       6),
    (jsonb_build_object('name','End Date',                 'data_type','date_mmddyyyy',               'mandatory',false,'user_fillable',true,'description','Leave blank when Pursuing'),             7),
    (jsonb_build_object('name','Completion Status Code *', 'data_type','code_picklist:COMPLETION_STATUS','mandatory',true,'user_fillable',true,'description','e.g. ES01=Completed ES02=Pursuing'),  8),
    (jsonb_build_object('name','Grade / GPA',              'data_type','text',                        'mandatory',false,'user_fillable',true),                                                       9),
    (jsonb_build_object('name','Highest Qualification',    'data_type','enum:Yes,No',                 'mandatory',false,'user_fillable',true,'description','At most one Yes per employee'),         10),
    (jsonb_build_object('name','Is Active',  'data_type','boolean',  'mandatory',false,'user_fillable',false,'include_with_system_metadata',true), 11),
    (jsonb_build_object('name','id',         'data_type','uuid',     'mandatory',false,'user_fillable',false,'include_with_system_metadata',true), 12),
    (jsonb_build_object('name','Created At', 'data_type','timestamp','mandatory',false,'user_fillable',false,'include_with_system_metadata',true), 13),
    (jsonb_build_object('name','Updated At', 'data_type','timestamp','mandatory',false,'user_fillable',false,'include_with_system_metadata',true), 14)
  ) AS t(col, ord)),
  'natural_key',   jsonb_build_array('Employee Code *','Education Level Code *','Institution *','Start Date *'),
  'row_processor', 'per_row',
  'notes',         'Education Level and Completion Status use picklist ref_id codes. Highest Qualification: at most one Yes per employee.'
), updated_at=NOW() WHERE template_code='education';
