-- =============================================================================
-- Migration 494 — Align mandatory fields across AddEmployee, WorkflowReview,
--                 and validate_hire_fields
--
-- MISMATCHES FOUND
-- ────────────────
-- Comparing AddEmployee wizard validation, get_employee_hire_review required
-- flags, and validate_hire_fields backend gate:
--
--   Field            AddEmployee  WorkflowReview  validate_hire_fields
--   ─────────────    ───────────  ──────────────  ────────────────────
--   last_name        required ✓   false      ✗    not checked ✗
--   personal_email   required ✓   true       ✓    not checked ✗
--   country_code     required ✓   true       ✓    not checked ✗
--   addr.state       optional ✗   true       ✗    not checked ✓
--   ec.relationship  required ✓   true       ✓    not checked ✗
--
-- FIXES
-- ─────
-- 1. get_employee_hire_review:
--    • last_name   required: false → true   (matches AddEmployee wizard)
--    • addr.state  required: true  → false  (matches AddEmployee: state is optional)
--
-- 2. validate_hire_fields:
--    • Personal:          add last_name IS NOT NULL
--    • Contact:           add personal_email + country_code IS NOT NULL
--    • Emergency Contact: add relationship IS NOT NULL
-- =============================================================================


-- =============================================================================
-- 1. get_employee_hire_review — fix required flags
-- =============================================================================
...
BEGIN
  ...
  v_result := v_result || jsonb_build_object(
    ...
    'fields', jsonb_build_array(
      ...
      jsonb_build_object('label','Last Name',      'value',COALESCE(v_last_name,         '—'),'raw_value',v_last_name,       'key','personal.last_name',     'editable',true, 'input_type','text',    'required',true),  -- mig 494: false → true
  ...
  IF v_addr.employee_id IS NOT NULL THEN
    v_result := v_result || jsonb_build_object(
      ...
      'fields', jsonb_build_array(
        ...
        jsonb_build_object('label','State',     'value',COALESCE(v_addr.state,    '—'),'raw_value',v_addr.state,    'key','addr.state',    'editable',true,'input_type','text','required',false),  -- mig 494: true → false
  ...
  LOOP
    ...
    v_edu_level_lbl  := NULL;
    v_edu_status_lbl := NULL;

    SELECT pv.value INTO v_edu_level_lbl
    FROM   picklist_values pv
    JOIN   picklists pl ON pl.id = pv.picklist_id
    WHERE  pl.picklist_id = 'EDUCATION_LEVEL'
      AND  pv.ref_id = v_edu_rec.education_level
    LIMIT 1;
    v_edu_level_lbl := COALESCE(v_edu_level_lbl, v_edu_rec.education_level);

    SELECT pv.value INTO v_edu_status_lbl
    FROM   picklist_values pv
    JOIN   picklists pl ON pl.id = pv.picklist_id
    WHERE  pl.picklist_id = 'COMPLETION_STATUS'
      AND  pv.ref_id = v_edu_rec.completion_status
    LIMIT 1;
    ...
    v_result := v_result || jsonb_build_object(
      ...
      'attachments', (
        ...
        WHERE a.education_id = v_edu_rec.id
      )
    );
  END LOOP;
...
COMMENT ON FUNCTION get_employee_hire_review(uuid) IS
  ...
  'Mig 406: Added Education sections. '
  'Mig 494: last_name required: false → true; addr.state required: true → false.';


-- =============================================================================
-- 2. validate_hire_fields — add missing required field checks
-- =============================================================================
...
BEGIN
  ...
  -- ── Personal: first_name + last_name + nationality + gender + dob + marital_status
  -- mig 494: added last_name (required in AddEmployee wizard and WorkflowReview)
  IF NOT EXISTS (
  ...
  -- ── Contact: mobile + personal_email + country_code ───────────────────────
  -- mig 494: added personal_email and country_code (required in both wizard and WFR)
  IF NOT EXISTS (
  ...
  -- ── Employment: read from satellite (mig 457/479) ─────────────────────────
  IF NOT EXISTS (
  ...
  -- mig 494: added relationship (required in AddEmployee wizard and WorkflowReview)
  IF NOT EXISTS (
...
COMMENT ON FUNCTION validate_hire_fields(uuid) IS
  'Validates all required hire sections. Raises check_violation listing every missing section. '
  'Mig 260: initial. Mig 431: added Bank, tightened Personal/Employment/Emergency. '
  'Mig 438: added marital_status + probation_end_date. '
  'Mig 457/479: Employment reads from satellite. '
  'Mig 494: added last_name, personal_email, country_code, ec.relationship — '
  'aligns backend gate with AddEmployee wizard and WorkflowReview required flags.';

DO $$
BEGIN
  RAISE NOTICE 'Migration 494: mandatory field alignment — '
               'last_name required in WFR + validate; '
               'state optional in WFR; '
               'personal_email, country_code, ec.relationship added to validate.';
END;
$$;
