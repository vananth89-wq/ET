-- =============================================================================
-- Migration 237: passport.country — correct UUID → label lookup
--
-- passports.country stores a UUID from the ID_COUNTRY picklist (same as
-- identity_records.country), NOT a plain country-name string.
-- Migration 236 was incorrect.  This patch adds the picklist lookup so the
-- read view shows the label ("India") while raw_value carries the UUID for
-- the select dropdown.
-- =============================================================================
...
DECLARE
  ...
  v_passport_country_lbl text;   -- ← new: resolved label for passport.country
...
BEGIN
  ...
  -- Resolve passport.country UUID → display label
  ...
  -- passport.country is a UUID FK to the ID_COUNTRY picklist (same as
  -- identity_records.country).  value = resolved label, raw_value = UUID.
  v_result := v_result || jsonb_build_object(
    'section', 'Passport',
    'fields', jsonb_build_array(
...
COMMENT ON FUNCTION get_employee_hire_review(uuid) IS
  ...
  'passport.country and id.*.country are both UUID FKs to ID_COUNTRY picklist — '
  'value = resolved label, raw_value = UUID for <select> pre-population.';
