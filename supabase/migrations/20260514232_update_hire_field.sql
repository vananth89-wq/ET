-- =============================================================================
-- Migration 232: update_hire_field — inline edit RPC for hire review
--
-- Called by approvers editing individual fields directly in WorkflowReview.
-- Routes the update to the correct satellite table based on the field key
-- returned by get_employee_hire_review.
--
-- FIELD KEY FORMAT
-- ────────────────
--   emp.<column>                    → employees
--   personal.<column>               → employee_personal  (upsert)
--   contact.<column>                → employee_contact   (upsert)
--   employment.<column>             → employee_employment (upsert)
--   addr.<column>                   → employee_addresses  (upsert)
--   passport.<column>               → passports           (UPDATE only)
--   ec.<column>                     → emergency_contacts  (UPDATE only)
--   id.<record_uuid>.<column>       → identity_records    (UPDATE by PK)
--
-- SECURITY
-- ────────
--   • Only applies to employees with status = 'Pending' (locked records).
--   • SECURITY DEFINER so authenticated users can update satellite tables
--     without direct table grants.
--   • Field key is matched with an explicit CASE, not interpolated into SQL,
...
CREATE OR REPLACE FUNCTION update_hire_field(
  p_employee_id uuid,
  p_field_key   text,
  p_new_value   text          -- always a string; dates cast inside
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
