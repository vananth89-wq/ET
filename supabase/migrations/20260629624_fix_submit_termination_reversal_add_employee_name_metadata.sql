-- =============================================================================
-- Migration 624: submit_termination_reversal — stamp employee_name in metadata
--
-- Bug: reversal workflow instances were missing employee_name in metadata,
-- so ApproverInbox showed the submitter name ("Vijey Aananth SR") instead of
-- the terminated employee's name ("Mohan Raj") in the card subtitle.
-- The primary termination flow (mig 575) stamps employee_name; reversals didn't.
--
-- Fix: add employee_name lookup + stamp it in wf_submit p_metadata.
-- All other logic is identical to mig 527.
-- =============================================================================
...
COMMENT ON FUNCTION submit_termination_reversal(uuid, jsonb, jsonb) IS
  'Mig 624: stamps employee_name in wf_submit metadata so ApproverInbox displays '
  'the terminated employee name instead of the submitter name. '
  'All other logic identical to mig 527.';
