-- =============================================================================
-- Migration 600: submit_termination — allow re-submission after reversal
--
-- Bug: the duplicate guard blocked new terminations when a REVERSED
-- termination already existed. REVERSED should be terminal (like WITHDRAWN /
-- REJECTED) — the employee was reinstated, so a fresh termination must be
-- allowed.
--
-- Fix: add 'REVERSED' to the NOT IN exclusion list in the duplicate guard
-- (step 6). Only change from mig 584 is that one line.
-- =============================================================================
...
BEGIN
  ...
  -- ── 6. Duplicate guard ─────────────────────────────────────────────────────
  -- REVERSED is now treated as terminal alongside WITHDRAWN and REJECTED.
  -- An employee whose termination was reversed is reinstated — a new
...
COMMENT ON FUNCTION submit_termination(uuid, jsonb, jsonb, text, jsonb) IS
  'Mig 600: REVERSED added to duplicate guard exclusion list — allows re-termination '
  'after a reversal. Otherwise unchanged from mig 584.';
