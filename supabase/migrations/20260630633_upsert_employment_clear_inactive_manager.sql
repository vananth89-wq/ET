-- =============================================================================
-- Mig 633: upsert_employment_info — null out inherited manager if inactive
--
-- When inserting a new employment slice (amendment / gap_fill / split / prepend),
-- the function inherits manager_id from the previous slice via COALESCE.
-- If that manager is now inactive (terminated), the inherited value should be
-- cleared rather than carried forward.
--
-- Fix: after case detection, if manager_id was NOT explicitly supplied in
-- p_proposed_data AND the slice is NOT a correction (i.e. it's a new slice),
-- check employees.status for the inherited manager. If 'Inactive', null out
-- v_target.manager_id so all COALESCE(v_manager_id, v_target.manager_id) calls
-- resolve to NULL.
--
-- Correction edits are excluded: when a user explicitly edits an existing
-- historical record they may intentionally keep or change the manager themselves.
-- =============================================================================
...
BEGIN
  ...
  -- ── 6b. FIX (mig 633): clear inherited manager if inactive ────────────────
  -- Only applies to NEW slices (not corrections of existing records).
  -- If manager_id was not explicitly provided, check whether the inherited
  -- manager (from v_target) is still active. If not, null them out.
  IF v_case != 'correction' AND v_manager_id IS NULL AND v_target.manager_id IS NOT NULL THEN
    IF EXISTS (
      SELECT 1 FROM employees
      WHERE id = v_target.manager_id AND status = 'Inactive'
    ) THEN
  ...
  -- FIX (mig 632): set bypass flag so the guard trigger allows this direct write.
