...
export default function Holidays() {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error: err } = await supabase
      .from('time_holidays')
      .select('id, holiday_code, holiday_name, created_at, updated_at, creator:profiles!created_by(first_name, last_name)')
      .order('holiday_code');
...
<td style={{ padding: '10px 12px', color: '#6B7280' }}>{h.creator ? `${h.creator.first_name ?? ''} ${h.creator.last_name ?? ''}`.trim() : '—'}</td>
                  <td style={{ padding: '10px 12px' }}>
