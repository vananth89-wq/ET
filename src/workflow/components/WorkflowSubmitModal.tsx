    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '18px 24px 0' }}>
          ...
          {/* CC row — de-duplicated by resolvedName (templates can list the
              same person via multiple CC roles, e.g. Initiator + Manager
              when the initiator is their own manager) */}
          {(() => {
            const seen = new Set<string>();
            const uniqueCc = ccParticipants.filter(p => {
              const key = (p.resolvedName ?? p.stepName ?? '').toLowerCase().trim();
