...
export async function buildApprovalExportData(p: TsPayload): Promise<TimesheetExportData> {
  ...
  const rows: AssembleRow[] = (p.entries ?? []).map(e => {
    ...
    return {
      ...
      // 801 leaves the booked project NULL and names the helped one in
      // related_project_id; 826 sends project_class, which is NULL for both
      ...
      // Mig 841 closed the gap this used to describe. 836 taught the payload
      // which project was helped; 830 records who asked; the two had never met,
      // so this was hardcoded null and the approver's copy printed the help
      // block without the one line 829 exists for -- the reader who can act on
      // it seeing less than the reader who cannot.
      requester:    e.requester ?? null,
